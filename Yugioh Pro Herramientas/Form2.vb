...
Public Class Form2
    ...
    Private Sub DataGridView1_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellDoubleClick
        PictureBox2.ImageLocation = ADMBD.Carpeta & "\pics\" & (DataGridView1.Rows(e.RowIndex).Cells(0).Value) & ".jpg"
        conexionbdsql("Select name from datas,texts where datas.id=texts.id and texts.id=" & DataGridView1.Rows(e.RowIndex).Cells(0).Value, txtNombre)
        conexionbdsql("Select desc from datas,texts where datas.id=texts.id and texts.id=" & DataGridView1.Rows(e.RowIndex).Cells(0).Value, txtDescripcion)
    End Sub
